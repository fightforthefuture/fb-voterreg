...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        invited_batches = orm.FriendshipBatch.objects.filter(invite_date__isnull=False)
        for fb in invited_batches:
            fb.friendship_set.all().update(invited_with_batch=True)

    def backwards(self, orm):
        for f in orm.Friendship.objects.all():
            f.invited_with_batch = False
            f.save()
